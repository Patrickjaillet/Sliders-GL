; =============================================================================
; Z-GL — NSIS Uninstaller Hooks
; File   : src-tauri/installer/uninstaller-hooks.nsh
; Purpose: Ask the user whether to also delete all Z-GL app data
...
;   %APPDATA%\com.zgl.editor\          ← Tauri store, updater state
;   %LOCALAPPDATA%\com.zgl.editor\     ← WebView2 user data (localStorage, cache)
;   %LOCALAPPDATA%\ZGL\                ← our custom install root

!define ZGL_APPDATA      "$APPDATA\com.zgl.editor"
!define ZGL_LOCALDATA    "$LOCALAPPDATA\com.zgl.editor"
!define ZGL_LOCALROOT    "$LOCALAPPDATA\ZGL"
...
Function un.AskRemoveAppData
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you also want to delete all Z-GL application data?$\n$\n\
This includes saved presets, editor settings, cached shaders,$\n\
window state, auto-save files, and logs.$\n$\n\
If you keep them, they will be restored if you reinstall Z-GL.$\n$\n\
...
Function un.RemoveAppDataIfConfirmed
  ${If} $RemoveAppData == "1"
    ...
    ; 4. Start Menu folder
    RMDir /r "$SMPROGRAMS\Z-GL"

    ; 5. Desktop shortcut
    Delete "$DESKTOP\Z-GL Shader Editor.lnk"
    ...
    DetailPrint "All Z-GL app data removed."

  ${Else}

    ; User chose NO — only remove shortcuts that point to the now-deleted exe.
    Delete "$SMPROGRAMS\Z-GL\Z-GL Shader Editor.lnk"
    RMDir  "$SMPROGRAMS\Z-GL"          ; remove folder only if empty
    Delete "$DESKTOP\Z-GL Shader Editor.lnk"
