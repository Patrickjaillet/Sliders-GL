...
fn set_window_title(app: AppHandle, title: String) {
    if let Some(win) = app.get_webview_window("main") {
        let _ = win.set_title(&format!("{title} — Sliders GL"));
    }
}
...
/// Wipes all Sliders GL user data from disk — called by the uninstaller confirmation
/// dialog (NSIS) and exposed to JS so the Settings page can offer a
/// "Reset everything" option without needing to uninstall.
///
/// Paths removed:
///   %APPDATA%\com.patrickjaillet.slidersgl          Tauri store
///   %LOCALAPPDATA%\com.patrickjaillet.slidersgl     WebView2 data partition (localStorage, cache)
///
...
fn clear_app_data(app: AppHandle) -> Result<Vec<String>, String> {
    ...
    let dirs: Vec<PathBuf> = {
        let mut v = Vec::new();
        if let Ok(p) = app.path().app_data_dir() { v.push(p); }       // %APPDATA%\com.patrickjaillet.slidersgl
        if let Ok(p) = app.path().app_local_data_dir() { v.push(p); } // %LOCALAPPDATA%\com.patrickjaillet.slidersgl
        if let Ok(p) = app.path().app_cache_dir() { v.push(p); }      // %LOCALAPPDATA%\com.patrickjaillet.slidersgl\cache
