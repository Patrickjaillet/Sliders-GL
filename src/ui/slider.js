...
// Pointer capture · Shift×0.1 fine / Ctrl×10 coarse · click-to-edit · hover
// nudge arrows · soft-range fill with hard-clamp edge feedback · RAF batching
...
// actual value is always hard-clamped to [e.min, e.max] (see _setVal). Falls
// back to the hard range when no soft bounds are set on the entry, so this is
// a no-op for every existing slider until soft-range metadata is populated
// (tracked separately under §3 "unit/step/decimal-precision metadata").
function _softRange(e) {
  const min = Number.isFinite(e.softMin) ? e.softMin : e.min;
  const max = Number.isFinite(e.softMax) ? e.softMax : e.max;
  return { min, max };
}
...
// ── Per-channel precise sliders — Blender's vector-property row ─────────────
// Shared by the colour swatch and the XY/vector pad: below the primary
// compact widget, render one fully precise, individually draggable/editable
// `.sl-field` (the exact same scrub-slider from §3's redesign — click-drag,
// Ctrl/Shift multipliers, hover nudge arrows, per-entry decimal precision)
// per component, labelled X/Y/Z/W or R/G/B/A. This is intentionally always
// visible rather than gated behind a disclosure toggle — matching how
// Blender's regular vector properties (Location XYZ, etc.) are shown by
// default, and keeping the interaction predictable (no hidden state to
// discover). Reuses the exact same `sl-<id>`/`sv-<id>` id scheme as a normal
// slider row, so it needs zero new wiring: `_attachSliderEvents`'s
// `.sl-field` query and the shared rAF fill/readout flush in `_setVal` pick
// these up automatically and stay in sync with the pad / colour picker,
// which already write through the same `onValChange(id, …)` path.
const _VEC_LABELS = ['X', 'Y', 'Z', 'W'];
...
<div class="sr sr-xy" id="sr-${gid}" title="${esc(ex.hint)}">
  ...
  ${_channelSlidersHTML(unit.entries, _VEC_LABELS)}
</div>`;
}
...
<div class="sr sr-color" id="sr-${gid}" title="${esc(run[0].hint)}">
  ...
  ${_channelSlidersHTML(run, _COLOR_LABELS)}
</div>`;
}
...
function _bindSvInput(id) {
  ...
  input.addEventListener('keydown', ev => {
    ...
    // keyboard accessibility: arrow keys nudge by step on every widget kind).
    if (ev.key === 'ArrowUp' || ev.key === 'ArrowRight') delta = +e.step;
    if (ev.key === 'ArrowDown' || ev.key === 'ArrowLeft') delta = -e.step;
...
function _initScrubField(field) {
  ...
  field.addEventListener('dblclick', (ev) => {
    if (ev.target.closest('.sl-nudge')) return;
    ev.preventDefault();
    focusForEdit();
  });
  ...
  field.addEventListener('pointerdown', (ev) => {
    ...
    // select the input's text at the same time. A plain click (no movement)
    // is exactly Blender's "click anywhere opens direct text-entry" — the
    // number fills the whole field, so there is no narrower hit-zone to
    // carve out; the click/drag distinction (see DRAG_THRESHOLD below) is
    // what tells the two interactions apart, same as Blender itself.
    ev.preventDefault();
    ...
    startX = ev.clientX;
    startVal = e.value;
    // Shift = ×0.1 fine/micro-step precision, Ctrl = ×10 large-step coarse
  ...
  field.addEventListener('pointermove', (ev) => {
    ...
    // Drag travels the *soft* range across the field width (Blender's
    // soft_min/soft_max) — the fill bar and this scaling share the same
    // range, so the handle tracks 1:1 with the filled bar under the cursor.
    const { min: softMin, max: softMax } = _softRange(e);
    let newVal;
    if (isLog) {
      const startFrac = _logPct(startVal, softMin, softMax) / 100;
      const newFrac = Math.max(0, Math.min(1, startFrac + (dx / rectW) * multiplier));
      newVal = _logValFromPct(newFrac, softMin, softMax);
    } else {
      newVal = startVal + (dx / rectW) * (softMax - softMin) * multiplier;
    }

    // Visual clamping feedback — a flat edge flash the moment the drag would
    // cross the *hard* min/max, independent of the soft range above. The
    // actual clamp always happens in _setVal, regardless of this class.
    field.classList.toggle('clamp-min', newVal < e.min);
    field.classList.toggle('clamp-max', newVal > e.max);
...
function _snapToStep(val, e) {
  return snapToStep(val, e.min, e.max, e.step);
}
