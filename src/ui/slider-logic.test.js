...
import { patchLine, fmtNum, setFromMonaco, snapToStep } from './slider-logic.js';
...
// ── snapToStep — high-precision slider step behavior (§3) ────────────────

describe('ui/slider-logic — snapToStep', () => {
  it('rounds to the nearest step relative to min', () => {
    expect(snapToStep(1.23, 0, 10, 0.5)).toBe(1);
    expect(snapToStep(1.3, 0, 10, 0.5)).toBe(1.5);
  });

  it('offsets the step grid when min is not zero', () => {
    // grid is min, min+step, min+2*step… so 1.06 with min=1, step=0.1 -> 1.1
    expect(snapToStep(1.06, 1, 5, 0.1)).toBeCloseTo(1.1, 10);
  });

  it('hard-clamps the snapped value to [min, max]', () => {
    expect(snapToStep(999, 0, 10, 1)).toBe(10);
    expect(snapToStep(-999, 0, 10, 1)).toBe(0);
  });

  it('falls back to a plain clamp when step is zero or negative', () => {
    expect(snapToStep(5.5, 0, 10, 0)).toBe(5.5);
    expect(snapToStep(15, 0, 10, -1)).toBe(10);
  });

  it('is idempotent — snapping an already-snapped value returns the same value', () => {
    const once = snapToStep(3.37, 0, 10, 0.25);
    expect(snapToStep(once, 0, 10, 0.25)).toBeCloseTo(once, 10);
  });
});
