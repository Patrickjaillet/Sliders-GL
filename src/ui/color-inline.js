...
export const VEC_RE =
  /\bvec([34])\(\s*([0-9.]+)\s*(?:,\s*([0-9.]+)\s*)?(?:,\s*([0-9.]+)\s*)?(?:,\s*([0-9.]+)\s*)?\)/g;
  ...
  for (let ln = 1; ln <= lineCount; ln++) {
    ...
    while ((m = VEC_RE.exec(text)) !== null) {
      ...
      if (comps.length === 1) {
        ...
        [r, g, b] = comps;
        if (dim === 4 && comps.length === 4) a = comps[3];
      } else continue;
      ...
      decos.push({
        ...
        options: {
          glyphMarginClassName: _injectClass(hex),
          glyphMarginHoverMessage: { value: `Color ${hex} — click to edit` },
          stickiness: 1,
        },
      });
    }
...
function _scanDebounced() {
  clearTimeout(_deb);
  _deb = setTimeout(_scan, 250);
}
...
function _openPicker(ln, info, clientX, clientY) {
  ...
  if (!document.getElementById('zgl-pop-style')) {
    ...
    st.id = 'zgl-pop-style';
    st.textContent = `
      /* §8 roadmap audit — box-shadow now uses --shadow-popup (== --elevation-3
         in tokens.css), same as every other floating popup in the app. */
      .zgl-color-pop{position:fixed;z-index:9999;background:var(--bg-surface,#cdcdcd);border:1px solid var(--border,#a8a8a8);border-radius:6px;padding:8px;display:flex;flex-direction:column;gap:5px;box-shadow:var(--shadow-popup, 0 8px 24px rgba(0,0,0,.28));min-width:180px;font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--prose,#242424);}
      .zgl-pop-row{display:flex;align-items:center;gap:6px;}
      #zgl-col-hex{flex:1;font-size:11px;color:var(--t3,#7e7e7e);}
      #zgl-col-eye{background:none;border:none;color:var(--t3,#7e7e7e);cursor:pointer;font-size:14px;padding:0 2px;}
      #zgl-col-eye:hover{color:var(--prose,#242424);}
      .zgl-hsl-row{display:flex;align-items:center;gap:5px;}
      .zgl-hsl-row label{width:12px;font-size:10px;color:var(--accent,#0b5650);font-weight:700;}
      .zgl-hsl-row input[type=range]{flex:1;accent-color:var(--accent,#0b5650);}
      .zgl-hsl-row span{width:32px;text-align:right;font-size:10px;color:var(--t3,#7e7e7e);}
    `;
...
function _onDocDown(e) {
  if (_pop && !_pop.contains(e.target)) _closePicker();
}
