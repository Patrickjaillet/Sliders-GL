...
describe('slider-customizations (roadmap Phase C)', () => {
  ...
  it('applyCustomizations overrides decimals / unit explicitly (not guessed)', () => {
    setSliderMeta('def:SPEED', { decimals: 4, unit: 'm/s' });
    const e = entry('def:SPEED', { decimals: 2 });
    const out = applyCustomizations([e]);
    expect(out[0].decimals).toBe(4);
    expect(out[0].unit).toBe('m/s');
  });

  it('applyCustomizations ignores a negative decimals override', () => {
    setSliderMeta('def:SPEED', { decimals: -1 });
    const e = entry('def:SPEED', { decimals: 2 });
    const out = applyCustomizations([e]);
    expect(out[0].decimals).toBe(2);
  });

  it('applyCustomizations allows decimals: 0 (explicit integer precision)', () => {
    setSliderMeta('def:SPEED', { decimals: 0 });
    const e = entry('def:SPEED', { decimals: 3 });
    const out = applyCustomizations([e]);
    expect(out[0].decimals).toBe(0);
  });

  it('applyCustomizations leaves decimals/unit untouched when not stored', () => {
    setSliderMeta('def:SPEED', { label: 'Vitesse' });
    const e = entry('def:SPEED', { decimals: 3 });
    const out = applyCustomizations([e]);
    expect(out[0].decimals).toBe(3);
