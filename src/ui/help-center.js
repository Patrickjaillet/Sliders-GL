 ...
 *            Channel Reference, Export Guide)
    ...
    chapters: [
      {
        ...
        anchor: 'what-is',
        content: `
<p>Z-GL is a real-time shader editor for Windows 10/11. You write GLSL fragment shaders and watch them render live at up to 120 fps — then export the result as video, image, standalone HTML, or a shareable URL.</p>
<p>Think of it as a <strong>ShaderToy desktop app</strong>: a Monaco-based GLSL editor, full multipass support (Image, Buffer A–D, Cube, Sound), and a full export pipeline — fully offline, no WASM.</p>
<h4>Key concepts</h4>
<ul>
  ...
  <li><strong>Channel</strong> — A texture input (<code>iChannel0</code>–<code>iChannel3</code>) you can wire to noise, images, video, audio FFT, webcam, etc.</li>
  <li><strong>Pass</strong> — One render target. The default <em>Image</em> pass writes to the screen. Add <em>Buffer A–D</em> passes for multi-pass effects.</li>
  <li><strong>Preset</strong> — A saved snapshot of shader code + channel config + slider values.</li>
      ...
      },
      {
        ...
        anchor: 'first-shader',
        content: `
<p>When Z-GL opens you will see the default example shader running. Here is how to write your own from scratch:</p>
<ol>
...
<p>Use the <strong>Snippets</strong> button (<kbd>Ctrl+K</kbd>) to insert ready-made templates for ray marching, SDFs, palettes, and more.</p>`
      },
      {
...
<table>
  ...
  <tr><td><kbd>Ctrl+K</kbd></td><td>Insert snippet</td></tr>
...
<ol>
  ...
  <li>Click <strong>Import</strong>. Z-GL fetches the code, maps all passes and channels, and starts rendering.</li>
</ol>
...
<ul>
  ...
  <li><strong>Auto-complete</strong> — Snippets for <code>mainImage</code>, <code>#define</code>, SDF primitives, ray march template, palettes.</li>
  ...
  <li><strong>Multi-cursor</strong> — <kbd>Alt+click</kbd> to add cursors, <kbd>Ctrl+D</kbd> to select next occurrence.</li>
      ...
      },
      {
        ...
        anchor: 'slider-panel',
        content: `
<p>Z-GL automatically generates a slider for every <code>#define</code> or <code>const float / int</code> at the top of your shader. Sliders let you tweak values in real time without recompiling.</p>
...
<table>
  ...
  <tr><td>Right-click</td><td>Context menu: Reset to default, Copy value, Rename, Set range…, Pin/Unpin</td></tr>
</table>
...
<p>Set the render rate to <strong>15 / 30 / 60 / 120 / Unlimited</strong> fps via the toolbar. The adaptive pixel ratio PID controller will also reduce resolution automatically under sustained GPU load.</p>`
      },
      {
        ...
        anchor: 'pass-graph',
        content: `
<p>Multi-pass rendering lets you chain shader passes, exactly like shadertoy.com — Buffer A can write to a texture that Buffer B reads as <code>iChannel0</code>, and so on.</p>
<h4>Switching passes</h4>
<p>Use the pass tabs above the editor (<strong>Image</strong>, <strong>Buf A–D</strong>, <strong>Cube A/B</strong>, <strong>Sound</strong>) to edit each pass's source separately. Click <strong>⌥ wire</strong> to open the wiring panel, which shows which buffer feeds which <code>iChannelN</code> for the active pass, and lets you reassign them.</p>
<h4>Pass types</h4>
<ul>
  <li><strong>Image</strong> — Final output, written to the screen.</li>
  <li><strong>Buffer A–D</strong> — Off-screen ping-pong render targets. Read the buffer's own previous frame via <code>iChannelN</code> (feedback) or read another buffer.</li>
  <li><strong>Cube A/B</strong> — Renders to a <code>textureCube</code> for environment mapping.</li>
  <li><strong>Sound</strong> — Generates audio via shader; plays back via <code>AudioWorkletNode</code>.</li>
</ul>`
      }
    ]
  },
...
<table>
  ...
  <tr><td><code>iSampleRate</code></td><td><code>float</code></td><td>Audio sample rate of the output device (typically 44100.0). Used in Sound passes.</td></tr>
</table>`
      },
      {
...
<table>
  <tr><th>Uniform</th><th>Type</th><th>Description</th></tr>
  <tr><td><code>iChannel0</code></td><td><code>sampler2D</code></td><td>Input texture channel 0. Type depends on the channel source configured in the Channels panel.</td></tr>
...
// Repeat vs Clamp — set per-channel in the Channels panel
// Default wrap mode is REPEAT</code></pre>`
      },
      {
        title: 'Z-GL Extended Uniforms',
        anchor: 'extended-uniforms',
        content: `
<p>Beyond the ShaderToy standard uniforms, setting a channel's type to <strong>keyboard</strong> makes that <code>iChannelN</code> a 256×2 key-state texture (ShaderToy <code>iKeyboard</code> convention) — row 0 = key currently pressed, row 1 = key toggled. Sample as <code>texture(iChannelN, vec2(keyCode/256.0, row)).x > 0.5</code>.</p>
...
<p><code>vec2</code>, <code>vec3</code>, <code>vec4</code>, <code>ivec2</code>, <code>ivec3</code>, <code>ivec4</code>, <code>mat2</code>, <code>mat3</code>, <code>mat4</code></p>`
  ...
  // ── 3 ── CHANNEL REFERENCE ───────────────────────────────────────────────
  {
    ...
    chapters: [
      {
        title: 'Channel Types Overview',
        anchor: 'channel-types',
        content: `
<p>Each of the four channel slots (<code>iChannel0</code>–<code>iChannel3</code>) can be wired to one of the following source types via the <strong>Channels panel</strong>.</p>
<table>
  <tr><th>Type</th><th>Description</th></tr>
  <tr><td><strong>proc-tex (noise)</strong></td><td>Procedural texture, default for an empty channel.</td></tr>
  <tr><td><strong>image</strong></td><td>Load a PNG, JPEG, or WebP from disk. Supports all standard wrap/filter modes.</td></tr>
  <tr><td><strong>video</strong></td><td>Play a local video file.</td></tr>
  <tr><td><strong>webcam</strong></td><td>Live webcam feed. Requires browser permission.</td></tr>
  <tr><td><strong>audio-mic</strong></td><td>Microphone FFT — frequency bins on the X axis (row 0) plus a raw waveform row (row 1).</td></tr>
  <tr><td><strong>audio-file</strong></td><td>FFT of a playing audio file. Configurable FFT size, smoothing, and window function.</td></tr>
  <tr><td><strong>prev-frame</strong></td><td>Feedback — the previous frame of the current Buffer pass. Use for blur, trails, or feedback effects.</td></tr>
  <tr><td><strong>cubemap</strong></td><td>Six-face cube texture. Shader must use <code>samplerCube</code>.</td></tr>
  <tr><td><strong>keyboard</strong></td><td>256×2 key state texture (ShaderToy <code>iKeyboard</code> compatible).</td></tr>
</table>
<p>A Buffer A–D pass's own output is wired as <code>prev-frame</code> from the <strong>⌥ wire</strong> panel — see the Multi-Pass chapter.</p>`
      },
      {
        title: 'Audio FFT Channel',
        anchor: 'audio-fft',
        content: `
<p>The audio FFT channel gives your shader access to the frequency spectrum of live audio or a playing audio file.</p>
<h4>Sampling the FFT</h4>
<pre><code>void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    // fragCoord.x maps to frequency (left=bass, right=treble), row 0 of the channel
    float freq = texture(iChannel0, vec2(fragCoord.x / iResolution.x, 0.0)).r;

    // Build a spectrum bar
    float bar = step(fragCoord.y / iResolution.y, freq);
    vec3 col = mix(vec3(0.0, 0.5, 1.0), vec3(1.0, 0.2, 0.0), freq);
    fragColor = vec4(col * bar, 1.0);
}</code></pre>
<p>Row 1 of the same texture holds the raw waveform instead of frequency data — sample at <code>y = 1.0</code> (normalized) to read it.</p>
<h4>Settings</h4>
<ul>
  <li><strong>FFT size</strong> — configurable bin count. Larger = more frequency resolution, less time resolution.</li>
  <li><strong>Smoothing</strong> — 0.0 (no smoothing) to 0.99 (very smooth). Reduces flicker but adds latency.</li>
  <li><strong>Window function</strong> — affects spectral leakage.</li>
</ul>`
      }
    ]
  },
...
<ul>
  ...
  <li><strong>↓ Download PNG</strong> saves the result.</li>
      ...
      },
      {
        ...
        anchor: 'html-export',
        content: `
<p>Export your shader as a single self-contained <code>.html</code> file that runs in any browser — no Z-GL Shadertoy required.</p>
<p>The exported file embeds your GLSL shader code, a minimal WebGL runtime, and all ShaderToy uniforms (<code>iTime</code>, <code>iMouse</code>, <code>iResolution</code>, etc.).</p>`
      },
      {
        ...
        content: `
<p>Use <strong>export → Code → ShaderToy format</strong> to strip local uniform/precision declarations so the result pastes directly into a shadertoy.com Image tab. To go the other way, drop or select a <code>.zip</code> exported from shadertoy.com — Z-GL Shadertoy reads the passes and wires up Buffer A–D / Sound / Cube automatically.</p>`
      },
      {
        title: 'Share Link',
        anchor: 'share-url',
        content: `
<p>Click <strong>share</strong> to generate a shareable URL. The shader code and slider values are compressed and encoded into the URL hash — <strong>no server is involved</strong>, the link is fully self-contained. <strong>Share Link v2</strong> (in the Code export tab) additionally encodes every multi-pass buffer.</p>`
      }
    ]
  }
];
...
function _buildPanel() {
  ...
  panel.setAttribute('aria-label', 'Z-GL Help Center');
  ...
  panel.innerHTML = `
    <div class="modal hc-modal" style="width:820px;max-width:96vw;height:600px;max-height:88vh;display:flex;flex-direction:column;padding:0;overflow:hidden;">
      ...
      <div class="hc-header" style="display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid var(--b1);flex-shrink:0">
        <span style="font-size:13px;font-weight:700;color:var(--spark);letter-spacing:.06em;font-family:var(--font-mono)">Z-GL</span>
      ...
      <div style="border-top:1px solid var(--b1);padding:7px 16px;display:flex;align-items:center;gap:12px;font-size:10px;color:var(--t3);font-family:var(--font-mono);flex-shrink:0">
        ...
        <div style="flex:1"></div>
        <span>Z-GL v2.12.0 · Offline docs</span>
      </div>
    </div>`;
