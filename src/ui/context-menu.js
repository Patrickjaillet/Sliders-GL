...
// ── Set range… (roadmap Phase E) ─────────────────────────────────────────────
// Small popover to edit a slider's min/max/step manually. The override persists
// via the Phase C customization store (survives reparse + saved with presets).
let _rangePop = null;
...
export function startRangeEdit(id) {
  ...
  pop.innerHTML = `
    <div class="rng-title">Range — ${escAttr(e.label)}</div>
  ...
  const apply = () => {
    ...
    setSliderMeta(e.stableKey, { min: lo, max: hi, step });
    buildUI(state.vars);
    _closeRangePop();
    toast('Range updated', 'ok');
  };
