...
export async function applyGLShader(code) {
  ...
  // succès sur ce GPU + cette version Sliders GL, on saute checkFragCompile() (coûteux).
  ...
  const chanUniforms = {};
  const newDummies = [];
  // External channel inputs were removed; iChannel0-3 start as dummy
  // textures (no multipass buffer wiring exists in this build).
  for (let i = 0; i < 4; i++) {
