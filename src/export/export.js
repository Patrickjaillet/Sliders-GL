...
function exportThreeSnippet() {
  const code = state.editor ? state.editor.getValue() : '';
  const snippet = `// Three.js ShaderMaterial snippet — generated by Z-GL
import * as THREE from 'three';
...
async function exportProjectZip() {
  ...
  const channels = (state.channels || []).map((ch, i) => ({
    ...
    type: ch?.type || 'none',
    flipY: !!ch?.flipY,
    gain: Number.isFinite(ch?.gain) ? ch.gain : 1,
    smoothing: Number.isFinite(ch?.smoothing) ? ch.smoothing : 0.8,
    band: ch?.band || 'both',
    wrap: ch?.wrap || 'repeat',
    src: ch?.src || '',
  }));
...
requestAnimationFrame(frame);
  ...
  for (let i = 0; i < (state.channels || []).length; i++) {
    ...
    const addBlob = async (url, filenameBase) => {
      try {
        if (!url) return;
        ...
        const ext = (type.split('/')[1] || 'bin').replace(/[^a-zA-Z0-9]/g, '');
        assetsFolder.file(`${filenameBase}.${ext || 'bin'}`, blob);
      } catch (e) {
      }
    };

    if (ch.type === 'image') {
      if (ch.src) await addBlob(ch.src, `channel${i}_image`);
    } else if (ch.type === 'video') {
      const src = (ch.videoEl && (ch.videoEl.currentSrc || ch.videoEl.src)) || ch.src;
      if (src) await addBlob(src, `channel${i}_video`);
    } else if (ch.type === 'audio-file') {
      if (ch.audioBlobUrl) await addBlob(ch.audioBlobUrl, `channel${i}_audio`);
      else if (ch.src) await addBlob(ch.src, `channel${i}_audio`);
    } else if (ch.type === 'cubemap' && Array.isArray(ch.cubeFaces)) {
      const names = ['px', 'nx', 'py', 'ny', 'pz', 'nz'];
      for (let f = 0; f < Math.min(6, ch.cubeFaces.length); f++) {
        await addBlob(ch.cubeFaces[f], `channel${i}_cube_${names[f] || f}`);
      }
    }
  }
