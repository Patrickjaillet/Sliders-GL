...
const BUILTIN_INCLUDES = {
  ...
  'math.glsl': `// Z-GL standard math utilities
#ifndef ZGL_MATH
#define ZGL_MATH
...
vec3 spherical(float theta, float phi) {
  ...
  'color.glsl': `// Z-GL color utilities
#ifndef ZGL_COLOR
#define ZGL_COLOR
...
vec3 acesFilmic(vec3 x) {
  ...
  'noise.glsl': `// Z-GL noise utilities
#ifndef ZGL_NOISE
#define ZGL_NOISE
...
float fbm(vec2 p) {
  ...
  'fbm.glsl': `// Z-GL Fractal Brownian Motion — parameterized via #include with {}
// \${OCTAVES} @default(5)
...
float fbm(vec2 p) {
  ...
  'sdf.glsl': `// Z-GL SDF primitives (Inigo Quilez)
#ifndef ZGL_SDF
#define ZGL_SDF
...
vec3 calcNormal(vec3 p, float d) {
  ...
  'raymarching.glsl': `// Z-GL ray marching template
