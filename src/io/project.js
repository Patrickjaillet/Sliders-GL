 ...
 *
 * Implements:
 *   - .zgl project format  (ZIP: shader.glsl + meta.json + channels/ + presets/)
 ...
 *   - Channel asset embedding on save
 ...
 * @param {() => object}               callbacks.getChannels
 * @param {(ch: object) => void}       callbacks.setChannels
...
export async function openProject(pathOverride) {
  ...
  if (!filePath) {
    const paths = await openFileDialog({
      filters: [
        { name: 'Z-GL Projects', extensions: ['zgl'] },
 ...
 * not a .zgl bundle (those are written by Z-GL itself).
 */
...
async function _doAutoSave() {
  ...
  try {
    ...
    const channels = _cb.getChannels();
    const payload = JSON.stringify({ path: _proj.path, name: _proj.name, code, channels,
...
async function _offerCrashRecovery() {
  ...
  setTimeout(() => {
    _cb.confirm(
      ...
      'Z-GL found an unsaved session from a previous run. Would you like to restore it?',
      async () => {
        try {
          ...
          if (data.channels) _cb.setChannels(data.channels);
...
async function _writeToDisk(filePath) {
  ...
  if (ext === 'glsl' || ext === 'frag' || ext === 'fs') {
    ...
    // Embed channel assets
    const embeddedChannels = await _embedChannelAssets(zip, channels);
...
/**
 * Copy referenced image/video channel assets into the ZIP bundle's `channels/`
 * folder and rewrite channel config to use relative paths.
 *
 * @param {JSZip} zip
 * @param {object} channels  — current channel config keyed by channel index
 * @returns {Promise<object>} rewritten channels config (relative paths for embedded assets)
 */
async function _embedChannelAssets(zip, channels) {
  const out = {};
  const channelsFolder = zip.folder('channels');

  for (const [key, ch] of Object.entries(channels || {})) {
    if (!ch || !ch.type || !ch.src) { out[key] = ch; continue; }
    // Only embed file-based channels (images, videos) that point to an absolute path
    const src = ch.src;
    const isAbsPath = /^[A-Za-z]:\\|^\//.test(src);
    if (!isAbsPath || !isTauri()) { out[key] = ch; continue; }

    try {
      const bytes = await readFileBytes(src);
      const fileName = src.split(/[\\/]/).pop();
      channelsFolder.file(fileName, bytes);
      out[key] = { ...ch, src: `channels/${fileName}`, embedded: true };
    } catch (err) {
      console.warn(`[project] could not embed channel ${key} asset "${src}":`, err);
      out[key] = ch;
    }
  }
