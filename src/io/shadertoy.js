...
async function doSTImport() {
  ...
  try {
    ...
    function resolvePassChannels(pass) {
      ...
      (pass.inputs || []).forEach(inp => {
        ...
        if (ctype === 'buffer') {
          ...
          let mapped = 'not supported, replaced with vec4(0.)';
          let texDesc = '';
      ...
      return { channelNotes, cubemapChannels, soundChannels, bufferChannelMap };
    }
    ...
    const extraPassNote = stExtraPassCount
      ? ` (${stExtraPassCount} extra pass${stExtraPassCount > 1 ? 'es' : ''} ignored — this build only supports a single Image pass)`
      : '';
