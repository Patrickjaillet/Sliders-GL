// Public JS API (window['Z-GL']) + embed/iframe mode
...
// Public JS API exposed on window['Z-GL']
window['Z-GL'] = {
  /**
  ...
  setUniform(nameOrId, value) {
    ...
    if (!e) { console.warn(`Z-GL.setUniform: unknown "${nameOrId}"`); return; }
  ...
  loadPreset(idOrName) {
    ...
    else console.warn(`Z-GL.loadPreset: unknown "${idOrName}"`);
  },

  /** Load raw GLSL code into the editor */
  setShader(glsl) {
    if (!state.editor) {
      console.warn('Z-GL.setShader called before editor is ready');
      return;
    }
...
function emitReady() {
  document.dispatchEvent(new CustomEvent('sl:ready', { detail: { api: window['Z-GL'] } }));
}
