...
describe('initial state shape', () => {
  ...
  it('multipass state only has the surviving "image" pass', () => {
    expect(state.mp.passes).toHaveProperty('image');
    expect(Object.keys(state.mp.passes)).toEqual(['image']);
  });

  it('image pass is enabled by default', () => {
