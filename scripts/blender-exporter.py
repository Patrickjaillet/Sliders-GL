#!/usr/bin/env python3
"""
Blender to Sliders GL Scene Exporter
Exports Blender scenes to Sliders GL scene format (.zgl-scene)
"""
...
def export_scene(filepath: str) -> None:
    ...
    print(f"[Sliders GL] Exported to {filepath} ({len(objects)} objects)")


def export_menu_func(self, context):
    default = os.path.splitext(bpy.data.filepath)[0] + ".zgl-scene" if bpy.data.filepath else "scene.zgl-scene"
    self.layout.operator("export_scene.zgl_scene", text="Sliders GL Scene (.zgl-scene)").filepath = default


class ExportZGLScene(bpy.types.Operator):
    """Export current scene to Sliders GL format"""
    bl_idname  = "export_scene.zgl_scene"
    bl_label   = "Export Sliders GL Scene"
    bl_options = {"REGISTER"}
