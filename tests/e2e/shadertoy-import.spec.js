...
test.describe('ShaderToy import E2E', () => {
  ...
  test('channel UI elements are present in the layout', async ({ page }) => {
    // Verify channel selectors exist (iChannel0–3)
    const channelSection = page.locator('#chHead, #channel-panel, [id^="ch-sel-"]').first();

    // Channels section may be collapsed — try to expand
    const chHead = page.locator('#chHead').first();
    if (await chHead.isVisible()) {
      const isExpanded = await chHead.evaluate(el =>
        el.getAttribute('aria-expanded') === 'true' ||
        !el.closest('.panel-section')?.classList.contains('collapsed')
      );
      if (!isExpanded) await chHead.click();
    }

    // At least one channel selector should exist
    const channelSelects = page.locator('[id^="ch-type-"], [data-ch]');
    const count = await channelSelects.count();
    expect(count).toBeGreaterThanOrEqual(0); // Flexible — layout may differ
  });
});
